 ...
 * 
 * This filter is modified to also shingle the part-of-speech tags. 
 */
public final class ShingleFilter extends TokenFilter {
  ...
  private StringBuilder posBuilder = new StringBuilder();
  private StringBuilder netBuilder = new StringBuilder();
  ...
  private final NETAttribute netAtt = addAttribute(NETAttribute.class);
  ...
  public final boolean incrementToken() throws IOException {
    ...
    if (inputWindow.size() >= gramSize.getValue()) {
      ...
      for (int gramNum = 1 ;
        ...
        if (builtGramSize < gramNum) {
          if (builtGramSize > 0) {
            ...
            posBuilder.append(tokenSeparator);
            netBuilder.append(tokenSeparator);
          }
          gramBuilder.append(nextToken.termAtt.buffer(), 0, 
                             nextToken.termAtt.length());
          posBuilder.append(nextToken.posAtt.getPartOfSpeech()); 
          netBuilder.append(nextToken.netAtt.getNET()); 
          ++builtGramSize;
        }
      ...
      if ( ! isAllFiller && builtGramSize == gramSize.getValue()) {
        ...
        posAtt.setPartOfSpeech(posBuilder.toString());
        netAtt.setNET(netBuilder.toString());
  ...
  private class InputWindowToken {
    ...
    final POSAttribute posAtt;
    final NETAttribute netAtt;
    ...
    public InputWindowToken(AttributeSource attSource) {
      ...
      this.posAtt = attSource.getAttribute(POSAttribute.class);
      this.netAtt = attSource.getAttribute(NETAttribute.class);
    }
  }
}
